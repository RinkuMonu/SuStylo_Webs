...
const payOut = async (req, res) => {
  try {
    ...
    if (existingPayout) {
      return res.status(409).json({ success: false, message: "A payout with this reference already exists." });
    }

    // Save payout request as pending
    ...
    return res.status(200).json({
      success: true,
      message: "Payout request saved successfully and wallet updated.",
      payout: newPayOut,
      wallet,
    });
  } catch (error) {
...
export { payOut, adminAction, callbackPayout, payOutReportAllUsers };